...
[rad4_ham, frq4, opt2] = airs2cris(rad2, frq2, sfile, opt1);
rad3 = opt2.brad;
bfrq = opt2.bfrq;

% option to apodize true CrIS
if hapod
  rad1_ham = hamm_app(rad1);
end
...
bt1_ham = real(rad2bt(frq1, rad1_ham));
...
axis([pv1 pv2 -1.5 1.5]); legend('unapodized','Hamming ap.','Location','North');grid on;
%xlabel('wavenumber'); 
ylabel('dBT (K)')
...
axis([pv1 pv2 0 0.]); legend('Unapodized','Hamming ap.','Location','North');
