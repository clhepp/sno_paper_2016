...
if hapod  app = 'hamm';   rad2_ham = hamm_app(rad2); end
if ~hapod app = 'noapp';  end
...
figure(1); clf; j = 1; 
plot(frq1, bt1(:,j), frq2, bt2(:,j), frq4, bt4_ham(:,j))
...
plot(frq2(i2), mean(bt4_ham(i4,:) - bt2_ham(i2,:),2),'LineWidth',1.5);
%ax = axis; ax(1)=pv1; ax(2)=pv2; axis(ax); xlabel('wavenumber'); 
axis([pv1 pv2 -0.045 0.045]);ylabel('dBT (mK)')
